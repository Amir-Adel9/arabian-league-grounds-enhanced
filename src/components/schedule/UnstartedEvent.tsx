...
const UnstartedEvent = ({
  ...
  const { data: currentPrediction } = useSWR<{
  ...
  return (
    <div className=' justify-between relative flex items-center w-full h-[120px] bg-card text-primary px-6 py-4 font-geist rounded-xl  duration-300 hover:bg-accent-blu'>
      <div
      ...
      >
        ...
        <h4 className='text-sm font-mono text-accent-gold font-bold'>APPROX</h4>
      </div>
      <div
        onClick={() => {
      ...
      >
        <div
          className={`absolute w-full opacity-0 h-full bg-gradient-to-r from-destructive duration-700 group-hover:opacity-100 ${
            selectedTeam === teams.firstTeam ||
            currentPrediction?.prediction.winningTeamId === teams.firstTeam.code
              ? 'opacity-100'
              : ''
          }`}
        ></div>
      ...
      >
        <div
          className={`absolute w-full opacity-0 h-full bg-gradient-to-l from-accent-blue duration-700 group-hover:opacity-100 ${
